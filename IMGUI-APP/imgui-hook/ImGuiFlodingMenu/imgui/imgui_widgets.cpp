...
void ImGui::FunctionItem(const char* title, const int color, const int bg_color, ImVec2 padding, bool open, bool selected) {
    va_list args;
    va_start(args, title);
    ...
    ImGuiContext& g = *GImGui;
    const char* text_end = g.TempBuffer + ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), title, args);
    const char* text_begin = g.TempBuffer;
    ...
    // Render (we don't hide text after ## in this end-user function)
    float height = (bb.Max - bb.Min).y;
    ImVec2 blockSize = { height,height };
    blockSize = blockSize * 0.6;

    float gap = 2.f;

    auto MaxRect = bb.Max + padding * 2.f;
    //背景颜色
    window->DrawList->AddRectFilled(bb.Min, { MaxRect.x + gap + blockSize.x, MaxRect.y }, bg_color);

    if(selected)
        window->DrawList->AddRect(bb.Min, { MaxRect.x + gap + blockSize.x, MaxRect.y }, 0xff0000ff);

    float d = (height - blockSize.y) / 2.f;
    int blockColor;
    if (open)
    ...
    //开启状态 方块
    window->DrawList->AddRectFilled({ bb.Min.x + padding.x, bb.Min.y + padding.y + d }, {bb.Min.x + padding.x + blockSize.x, bb.Min.y + d + padding.y + blockSize.y}, blockColor);
    //功能的名称
    window->DrawList->AddText( { bb.Min.x + padding.x + blockSize.x + gap, bb.Min.y + padding.y}, color, text_begin, text_end);

    va_end(args);
}

IMGUI_API void ImGui::Function(const char* title, const int color, const int bg_color, ImVec2 padding, bool open, bool selected)
{
    va_list args;
    va_start(args, title);
    ...
    auto MaxRect = bb.Max + padding * 2.f;
    //背景颜色
    window->DrawList->AddRectFilled(bb.Min, MaxRect, bg_color);

    if (selected)
        window->DrawList->AddRect(bb.Min, MaxRect, 0xff0000ff);

    //功能的名称
    window->DrawList->AddText({ bb.Min.x + padding.x + gap, bb.Min.y + padding.y }, color, text_begin, text_end);


    va_end(args);
}

IMGUI_API void ImGui::Gap()
{
